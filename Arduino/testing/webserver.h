...
struct DeviceState {
  ...
  float power;                                            // measured power   (0–100)
  float pressure;                                         // measured pressure (0–100)
  ...
  float powerSetpoint;     // 0–100
  float pressureSetpoint;  // 0–100
  ...
  DeviceState()
    : power(0), pressure(0),
      ...
      powerSetpoint(0.0), pressureSetpoint(0.0),
...
public:
  ...
  void pushStatusMessage(const char* msg) {
    for (int i = 0; i < STATUS_MSG_COUNT - 1; i++)
      memcpy(_state.statusMessages[i], _state.statusMessages[i + 1], STATUS_MSG_LEN);
    strncpy(_state.statusMessages[STATUS_MSG_COUNT - 1], msg, STATUS_MSG_LEN - 1);
    _state.statusMessages[STATUS_MSG_COUNT - 1][STATUS_MSG_LEN - 1] = '\0';
  }

  /** Set measured power reading (0–100). */
  void setPower(float value) {
    _state.power = constrain(value, 0, 100);
  }

  /** Set measured pressure reading (0–100). */
  void setPressure(float value) {
    _state.pressure = constrain(value, 0, 100);
  }
  ...
  float getPowerSetpoint() const {
    return _state.powerSetpoint;
  }
  float getPressureSetpoint() const {
    return _state.pressureSetpoint;
  }

  /** Returns the Unix timestamp sent by the last browser time-sync, or 0. */
  uint32_t getClientEpoch() const {
    return _state.clientEpoch;
  }
  ...
  bool hasNewClientTime() {
    if (_state.timeUpdated) {
      _state.timeUpdated = false;
      return true;
    }
    return false;
  }
...
private:
...
#endif
  {
    ...
    Serial.println("[WS] " + methodStr + " " + pathStr + " from " + clientIP.toString());
  ...
  void _parseControlRequest(const String& body) {
    ...
    if (_req.containsKey("mode")) {
      ...
      _state.mode = (ControlMode)constrain(m, 0, 7);
    }
    ...
    Serial.println("[WS] Control update -> mode=" + String((int)_state.mode) + " pwr=" + String(_state.powerSetpoint) + " pres=" + String(_state.pressureSetpoint));
  }
  ...
  String _buildCpuHtml(const String& baseUrl) {
    String h = "<!DOCTYPE html><html><head>"
               ...
               "body{font-family:monospace;font-size:14px;color:#fff;background:#111;padding:12px;max-width:480px}"
               "@media(prefers-color-scheme:dark){body{background:#fff;color:#eee}.card{background:#1e1e1e}.badge{border-color:#444;color:#aaa}.msg,.row{border-color:#2a2a2a}}"
    ...
    h += "<div class='readings'>"
         "<div class='card'><div class='lbl' style='font-size:11px;color:#888'>Leistung</div>"
         "<div class='n' id='pwr'>—</div><div class='u'>%</div>"
         "<div class='bar-wrap'><div class='bar' id='pwrB'></div></div></div>"
         "<div class='card'><div class='lbl' style='font-size:11px;color:#888'>Druck</div>"
  ...
  String _buildHmiHtml(const String& baseUrl) {
    String h = "<!DOCTYPE html><html><head>"
               ...
               "*{box-sizing:border-box;margin:0;padding:0}"
               "body{font-family:monospace;font-size:14px;color:#fff;background:#111;padding:12px;max-width:480px}"
               "@media(prefers-color-scheme:dark){body{background:#fff;color:#eee}input[type=range]{accent-color:#eee}}"
               "h2{font-size:15px;font-weight:500;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #ddd}"
               "@media(prefers-color-scheme:dark){h2{border-color:#2a2a2a}}"
    ...
    h += "<a href='" + baseUrl + "/'>zu den Anlageninformationen</a>";
    h += "<h2>Modus</h2>";
    h += "<div class='modes'>"
         "<button id='b0' onclick='setMode(1)'>Stop</button>"
         "<button id='b1' onclick='setMode(2)'>Leistungsregelung</button>"
         "<button id='b2' onclick='setMode(3)'>Druckregelung</button>"
         "<button id='b3' onclick='setMode(4)'>Leistungsregelung (Nacht)</button>"
         "<button id='b4' onclick='setMode(5)'>Druckregelung (Nacht)</button>"
         "<button id='b5' onclick='setMode(6)'>Speicherbetrieb</button>"
         "</div>";
    h += "<div class='sp' id='sp'>"
         ...
         "</div>";
    h += "<h2>Time</h2>";
    h += "<button class='tbtn' onclick='syncTime()'>Zeit synchronisieren</button>";
    ...
    h += "<script>const B='" + baseUrl + "';"
                                         ...
                                         "lb.childNodes[0].textContent=m===1?'Leistung setpoint':'Druck setpoint ';}"
                                         "send();}"
                                         "async function send(){"
